...
static const char *motif_resourceNames[] = {"XmNnull",
...
void _Gui_callCallbacks (
        GuiObject w, XtCallbackList *callbacks, XtPointer call) {
...
static GuiObject
        theMenus[1 + MAXIMUM_NUMBER_OF_MENUS];   // we can freely use and reuse
                                                 // these menu ids
static char32 theWindowClassName[100], theDrawingAreaClassName[100],
        theApplicationClassName[100];
...
static short
        theMenuItems[1 + MAXIMUM_MENU_ITEM_ID];   // we can freely use and reuse
                                                  // the item ids 4001..32767

static GuiObject theApplicationShell;   // for global menus
static int theBackground = False;   // set by suspend and resume events; used by
                                    // Motif-style activation methods
static int theDialogHint = False;   // should the shell that is currently being
                                    // created, have dialog or document looks?
...
static void cb_scroll (GuiObject scrollBar, XtPointer closure, XtPointer call) {
	...
	bool horizontal =
	        (scrolledWindow->motiff.scrolledWindow.horizontalBar == scrollBar);
	(void) call;
	if (!workWindow) return;
	Melder_assert (scrolledWindow->motiff.scrolledWindow.clipWindow);
	previousShift =
	        horizontal ? scrolledWindow->motiff.scrolledWindow.clipWindow->x -
	                             workWindow->x
	                   : scrolledWindow->motiff.scrolledWindow.clipWindow->y -
	                             workWindow->y;
...
GuiObject _Gui_initializeWidget (
        int widgetClass, GuiObject parent, conststring32 name) {
	GuiObject me = Melder_calloc_f (struct structGuiObject, 1);
	if (Melder_debug == 34)
		Melder_casual (U"from _Gui_initializeWidget\t", Melder_pointer (me),
		        U"\t1\t", sizeof (struct structGuiObject));
	...
	*/
	...
	/*
	    I am in the same shell as my parent, so I inherit my parent's "shell"
	   attribute.
	*/
	if (MEMBER (me, Shell))
		my shell = me;
	...
	/*
	 * The remainder of initialization is about positioning, sizes, attachments,
	 * and the contents of a scrolled window. All of that is irrelevant to menu
	 * items.
	 */
	...
	case xmCascadeButtonWidgetClass: {
		if (my parent->rowColumnType == XmMENU_BAR) {
			...
			my height = NativeButton_preferredHeight (me) +
			            4;   // BUG: menu bar should be large enough
		} else {
	...
	case xmFormWidgetClass: {
		if (MEMBER (parent, Shell)) {
			/*
			 * The following trick is necessary for forms that contain scroll
			 * bars.
			 */
			my width = parent->width;
			...
			/*
			 * EXPERIMENT:
			 * this relies on a certain sequence in building up a window: from
			 * top to bottom. If the form is inside a RowColumn of a certain
			 * width established by a widget closer to the top, this width will
			 * be copied. The height, of course, should not be copied. See
			 * Praat's SoundRecorder for an example.
			 */
			my width = parent->width;
			my height = 10;
		}
	...
	if (parent) switch (parent->widgetClass) {
		...
		case xmScrolledWindowWidgetClass: {
			...
			my y = 1;
			my width = parent->width -
			           17;   // exact fit: scroll bar (16) plus border (1)
	...
	if (MEMBER2 (me, BulletinBoard, Form) && MEMBER (my parent, Shell))
		my leftAttachment = my rightAttachment = my topAttachment =
		        my bottomAttachment = XmATTACH_FORM;

	if (MEMBER (me, CascadeButton) && str32equ (name, U"Help"))
		my rightAttachment = XmATTACH_FORM; /* !!!!!! */

	/* A child of a scrolled window will be installed as the workWindow of that
	 * scrolled window, except if it is a scroll bar or if the clipWindow does
	 * not exist yet. This is because the creations of the scroll bars and the
	 * clip window will also arrive here. Our XmScrolledWindow creation method
	 * always creates two scroll bars and a clip window, before you can create
	 * any other children.
	 */

	if (my parent && MEMBER (my parent, ScrolledWindow) &&
	        !MEMBER (me, ScrollBar) &&   // 'me' is one of the two scroll bars,
	                                     // or a new one
	        my parent->motiff.scrolledWindow
	                .clipWindow   // 'me' is probably the clip window now
	)
...
void _GuiNativeControl_destroy (GuiObject me) { DestroyWindow (my window); }

void _GuiNativeControl_show (GuiObject me) { ShowWindow (my window, SW_SHOW); }

void _GuiNativeControl_hide (GuiObject me) { ShowWindow (my window, SW_HIDE); }
...
void _GuiNativeControl_setTitle (GuiObject me) {
	...
	SetWindowTextW (my window,
	        Melder_peek32toW (_GuiWin_expandAmpersands (my name.get ())));
}

static int _XmScrollBar_check (GuiObject me) {
	if (my maximum < my minimum)
		Melder_warning (U"XmScrollBar: maximum (", my maximum,
		        U") less than minimum (", my minimum, U").");
	else if (my sliderSize > my maximum - my minimum)
		Melder_warning (U"XmScrollBar: slider size (", my sliderSize,
		        U") greater than maximum (", my maximum, U") minus minimum (",
		        my minimum, U").");
	else if (my value < my minimum)
		Melder_warning (U"XmScrollBar: value (", my value,
		        U") less than minimum (", my minimum, U").");
	else if (my value > my maximum - my sliderSize)
		Melder_warning (U"XmScrollBar: value (", my value,
		        U") greater than maximum (", my maximum,
		        U") minus slider size (", my sliderSize, U").");
	else
		return 1;
	return 0;
}
...
static void NativeScrollBar_set (GuiObject me) {
	...
	if (my maximum == my minimum + my sliderSize) {
		...
		scrollInfo.nPage =
		        (32767.0 * my sliderSize) / (my maximum - my minimum);
		scrollInfo.nPos =
		        ((32767.0 - scrollInfo.nPage) * (my value - my minimum)) /
		        (my maximum - my minimum - my sliderSize);
	}
...
static int NativeMenuItem_getPosition (GuiObject me) {
	 ...
	 */
	GuiObject sibling;
	for (sibling = my parent->firstChild; sibling;
	        sibling = sibling->nextSibling) {
		if (sibling == me) break;
		if (sibling->managed && !MEMBER (sibling, PulldownMenu)) position += 1;
	}
	/*
...
static void NativeMenuItem_check (GuiObject me, Boolean value) {
	...
	CheckMenuItem (my nat.entry.handle, my nat.entry.id,
	        MF_BYCOMMAND | (value ? MF_CHECKED : MF_UNCHECKED));
}
...
static void NativeMenuItem_setSensitive (GuiObject me) {
	...
	if (my widgetClass == xmPulldownMenuWidgetClass) {
		...
		EnableMenuItem (my parent->nat.menu.handle, my nat.menu.id,
		        MF_BYCOMMAND | (my insensitive ? MF_GRAYED : MF_ENABLED));
	} else {
		EnableMenuItem (my nat.entry.handle, my nat.entry.id,
		        MF_BYCOMMAND | (my insensitive ? MF_GRAYED : MF_ENABLED));
	}
...
static void _GuiNativizeWidget (GuiObject me) {
	...
	if (my inMenu) {
		if (MEMBER (me, PulldownMenu)) {
			...
			/*
			 * Any menu item (push button, toggle button, or cascade button)
			 * shall contain its native parent menu handle.
			 */
			my nat.entry.handle = my parent->nat.menu.handle;
			{
				 ...
				 * This ID should be higher than 4000, in order to be different
				 * from the menu IDs. In our implementation, item IDs are
				 * application-unique.
				 */
				int id;
				for (id = MINIMUM_MENU_ITEM_ID; id <= MAXIMUM_MENU_ITEM_ID;
				        id++)
					if (!theMenuItems[id]) break;
		...
		case xmBulletinBoardWidgetClass: {
			my window = CreateWindowEx (0,
			        ...
			        WS_CHILD | WS_BORDER | WS_CLIPSIBLINGS, my x, my y,
			        my width, my height, my parent->window, NULL,
		...
		case xmPulldownMenuWidgetClass: {
			...
			if (MEMBER (my parent, MenuBar)) {
				...
				my nat.menu.handle = CreatePopupMenu ();
				/*
				 * Insert the menu before the Help menu, if that exists;
				 * otherwise, at the end.
				 */
				for (menu = my parent->firstChild; menu;
				        menu = menu->nextSibling) {
					if (MEMBER (menu, PulldownMenu) &&
					        str32equ (menu->name.get (), U"Help") &&
					        menu != me) {
				...
				}
				{
					...
					info.fType =
					        MFT_STRING | (str32equ (my name.get (), U"Help")
					                                     ? MFT_RIGHTJUSTIFY
					                                     : 0);
					info.dwTypeData =
					        (mutablestringW) Melder_peek32toW (my name.get ());
					info.hSubMenu = my nat.menu.handle;
					info.wID = (UINT) my nat.menu./*handle*/ id;
					InsertMenuItem (
					        my parent->nat.menu.handle, beforeID, 0, &info);
				} /*else if (beforeID >= 0)
				    InsertMenu (my parent -> nat.menu.handle, beforeID,
				MF_STRING | MF_POPUP | MF_BYCOMMAND, (UINT) my nat.menu.handle,
				my name); else AppendMenu (my parent -> nat.menu.handle,
				MF_STRING | MF_POPUP, (UINT) my nat.menu.handle, my name);*/
				DrawMenuBar (my shell->window);
		...
		case xmCascadeButtonWidgetClass: {
			if (my motiff.cascadeButton.inBar) {
				my nat.entry.handle =
				        my parent->nat.menu.handle;   // TODO: superfluous?
			} else {
				my window = CreateWindow (L"button",
				        Melder_peek32toW (
				                _GuiWin_expandAmpersands (my name.get ())),
				        WS_CHILD | BS_PUSHBUTTON | WS_CLIPSIBLINGS, my x, my y,
				        my width, my height, my parent->window, (HMENU) 1,
				        theGui.instance, NULL);
				SetWindowLongPtr (my window, GWLP_USERDATA, (LONG_PTR) me);
		...
		case xmScaleWidgetClass: {
			my window = CreateWindow (PROGRESS_CLASS,
			        Melder_peek32toW (
			                _GuiWin_expandAmpersands (my name.get ())),
			        WS_CHILD | WS_CLIPSIBLINGS, my x, my y, my width, my height,
			        my parent->window, (HMENU) 1, theGui.instance, NULL);
			SetWindowLongPtr (my window, GWLP_USERDATA, (LONG_PTR) me);
			SendMessage (my window, PBM_SETRANGE, (WPARAM) 0,
		...
		case xmScrollBarWidgetClass: {
			my window = CreateWindow (L"scrollbar",
			        ...
			        WS_CHILD |
			                (str32equ (my name.get (), U"verticalScrollBar")
			                                ? SBS_VERT
			                                : SBS_HORZ) |
			                WS_CLIPSIBLINGS,
			        my x, my y, my width, my height, my parent->window,
			        (HMENU) 1, theGui.instance, NULL);
			SetWindowLongPtr (my window, GWLP_USERDATA, (LONG_PTR) me);
		...
		case xmScrolledWindowWidgetClass: {
			/*
			 * The space of the scrolled window is completely filled by three of
			 * its children: the two scroll bars and the clip window. The first
			 * child you create yourself will be the work window: this work
			 * window can grow larger than the clip window. While normally every
			 * widget is only clipped to the rects of its ancestors, for a
			 * scrolled window the clip window will be inserted into this chain.
			 ...
			 */
			my window = CreateWindowEx (0,
			        Melder_peek32toW (theWindowClassName), L"scrolledWindow",
			        WS_CHILD | WS_CLIPSIBLINGS, my x, my y, my width, my height,
			        my parent->window, NULL, theGui.instance, NULL);
			SetWindowLongPtr (my window, GWLP_USERDATA, (LONG_PTR) me);
			my motiff.scrolledWindow.horizontalBar =
			        XmCreateScrollBar (me, "horizontalScrollBar", NULL, 0);
			my motiff.scrolledWindow.verticalBar =
			        XmCreateScrollBar (me, "verticalScrollBar", NULL, 0);
			XtVaSetValues (my motiff.scrolledWindow.horizontalBar,
			        XmNorientation, XmHORIZONTAL, XmNleftAttachment,
			        XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM,
			        XmNrightAttachment, XmATTACH_FORM, XmNrightOffset, 15,
			        XmNheight, 16, XmNminimum, 0, XmNmaximum, 100,
			        XmNsliderSize, 100, XmNincrement, CELL_HEIGHT,
			        XmNpageIncrement, 101 - CELL_HEIGHT, NULL);
			XtVaSetValues (my motiff.scrolledWindow.verticalBar, XmNorientation,
			        XmVERTICAL, XmNtopAttachment, XmATTACH_FORM,
			        XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
			        XmATTACH_FORM, XmNbottomOffset, 15, XmNwidth, 16,
			        XmNminimum, 0, XmNmaximum, 100, XmNsliderSize, 100,
			        XmNincrement, CELL_HEIGHT, XmNpageIncrement,
			        101 - CELL_HEIGHT, NULL);
			my motiff.scrolledWindow.clipWindow =
			        XmCreateBulletinBoard (me, "clipWindow", NULL, 0);
			XtVaSetValues (my motiff.scrolledWindow.clipWindow,
			        ...
			        1,   // for border
			        XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 16,
			        NULL);   // for scroll bar
			XtAddCallback (my motiff.scrolledWindow.verticalBar,
			        XmNvalueChangedCallback, cb_scroll, (XtPointer) me);
			XtAddCallback (my motiff.scrolledWindow.verticalBar,
			        XmNdragCallback, cb_scroll, (XtPointer) me);
			XtAddCallback (my motiff.scrolledWindow.horizontalBar,
			        XmNvalueChangedCallback, cb_scroll, (XtPointer) me);
			XtAddCallback (my motiff.scrolledWindow.horizontalBar,
			        XmNdragCallback, cb_scroll, (XtPointer) me);
		} break;
		case xmShellWidgetClass: {
			static char32 *className{
			        theApplicationClassName};   // only for first window
			my window = CreateWindowEx (
			        theDialogHint ? WS_EX_DLGMODALFRAME /* | WS_EX_TOPMOST */
			                      : 0,
			        Melder_peek32toW (className), Melder_peek32toW (className),
			        theDialogHint ? WS_CAPTION | WS_SYSMENU
			                      : WS_OVERLAPPEDWINDOW,
			        CW_USEDEFAULT, 0, CW_USEDEFAULT, 0,
			        my parent ? my parent->window : NULL, NULL, theGui.instance,
			        NULL);
			className = theWindowClassName;   // all later windows
			SetWindowLongPtr (my window, GWLP_USERDATA, (LONG_PTR) me);
			my motiff.shell.isDialog =
			        theDialogHint;   // so we can maintain a single Shell class
...
void _Gui_invalidateWidget (GuiObject me) {
	...
	if (MEMBER (me, Shell) /*||
		 my widgetClass == xmRowColumnWidgetClass ||
		 my widgetClass == xmFormWidgetClass*/)
		return;   // composites are not invalidated !!!!! ????
}
...
static void Native_move (GuiObject me, int dx, int dy) {
	/* Native_move () changes the native attributes and visual position of the
	 * widget: No Motif attributes (such as 'x' and 'y') are changed. Usage:
	 *     Native_move () is normally called immediately after changing the 'x'
	 * or 'y' attribute. It can be seen as the 'realization' of a Motif move.
	 */
	(void) dx;
	(void) dy;
	...
	if (MEMBER (me, Shell)) {
		...
		if (my motiff.shell.isDialog) {
			...
			MoveWindow (my window, my x, my y,
			        ...
			        my height + 2 * GetSystemMetrics (SM_CYFIXEDFRAME) +
			...
			MoveWindow (my window, my x, my y,
			        ...
			        my height + 2 * GetSystemMetrics (SM_CYSIZEFRAME) +
			                GetSystemMetrics (SM_CYCAPTION) +
			                (my nat.shell.menuBar ? GetSystemMetrics (SM_CYMENU)
			                                      : 0),
			        true);
	} else
...
static void shellResizeWidget (GuiObject me, int dx, int dy, int dw, int dh) {
	...
	if (my window && !MEMBER (me, Shell)) {
		...
		if (rect.left!=my x||rect.right!=my x + my width||rect.top !=my
		y||rect.bottom!=my y +my height) Melder_warning ("class %d x %d left %d
		y %d top %d width %d right %d height %d bottom %d", my widgetClass, my
		x, rect.left, my y, rect.top, my width, rect.right, my height,
		rect.bottom);*/
		MoveWindow (my window, my x, my y, my width, my height, true);
	...
	for (child = my firstChild; child; child = child->nextSibling) {
		...
		if (child->leftAttachment == XmATTACH_POSITION &&
		        child->rightAttachment == XmATTACH_POSITION) {
		...
		if (child->topAttachment == XmATTACH_POSITION &&
...
static void _motif_setValues (GuiObject me, va_list arg) {
	...
	while (resource = va_arg (arg, int), resource != 0) switch (resource) {
		...
		case XmNdeleteResponse:
			...
			if (my deleteResponse == XmDO_NOTHING &&
			        !my motiff.shell.goAwayCallback) {
		...
		case XmNheight:
			...
			if (MEMBER (me, Shell)) {
				int maximumHeight = GetSystemMetrics (SM_CYMAXIMIZED) -
				                    GetSystemMetrics (SM_CYCAPTION) -
				                    GetSystemMetrics (SM_CYSIZEFRAME) -
				                    GetSystemMetrics (SM_CYMENU) - 15;
		...
		case XmNhorizontalScrollBar: {
			...
			XtVaSetValues (my motiff.scrolledWindow.horizontalBar,
			        ...
			        XmNheight, 16, NULL);
			/*
			 * Note that we may now have a horizontal bar with
			 * application-defined scrolling policy, and a vertical bar with
			 * automatic scrolling policy. This is more flexible than X Motif,
			 * but may be simulated in X Motif with the following strategy,
			 * which is compatible with MotifEmulator:
			 *   - set the scrolling policy to XmAUTOMATIC: two scroll bars will
			 * be created;
			 *   - ask for the horizontal scroll bar with XtVaGetValues;
			 *   - remove all the callbacks from the horizontal scroll bar with
			 * XtRemoveAllCallbacks;
			 *   - add your application-defined callback with XtAddCallback.
			 * We just hope now that X Motif does not look into the
			 * XmNscrollingPolicy resource after this...
			 */
		} break;
		case XmNincrement:
		...
		case XmNverticalScrollBar: {
			...
			XtVaSetValues (my motiff.scrolledWindow.verticalBar, XmNorientation,
			        XmVERTICAL, XmNtopAttachment, XmATTACH_FORM,
			        XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
			        XmATTACH_FORM, XmNbottomOffset, 15, XmNwidth, 16, NULL);
		} break;
		case XmNwidth:
			my width = va_arg (arg, int);
			resize = True;
		...
		default: {
			if (resource < 0 ||
			        resource >= sizeof motif_resourceNames / sizeof (char *))
	...
	if (attach) {
		...
		if (my leftAttachment == XmATTACH_POSITION &&
		        my rightAttachment == XmATTACH_POSITION) {
		...
		if (my topAttachment == XmATTACH_POSITION &&
	...
	if (move) Native_move (me, 0, 0);
	if (resize) resizeWidget (me, my width - oldWidth, my height - oldHeight);
...
void _Gui_manageScrolledWindow (GuiObject me) {
	...
	GuiObject clipWindow, workWindow,
	        horzBar = my motiff.scrolledWindow.horizontalBar,
	        vertBar = my motiff.scrolledWindow.verticalBar;
	 ...
	 */
	horzAutomatic =
	        horzBar->motiff.scrollBar.valueChangedCallbacks.pairs[0].proc ==
	        cb_scroll;
	vertAutomatic =
	        vertBar->motiff.scrollBar.valueChangedCallbacks.pairs[0].proc ==
	        cb_scroll;
	/*
	 * If the work window has been unmanaged or destroyed, the automatic scroll
	 * bars should be empty and insensitive.
	 */
	if (workWindow == NULL || !workWindow->managed) {
		if (horzAutomatic)
			XtVaSetValues (horzBar, XmNmaximum, 100, XmNsliderSize, 100,
			        XmNvalue, 0, XmNpageIncrement, 1, NULL);
		if (vertAutomatic)
			XtVaSetValues (vertBar, XmNmaximum, 100, XmNsliderSize, 100,
			        XmNvalue, 0, XmNpageIncrement, 1, NULL);
		return;
	}
	...
	/*
	 * If the scroll bar is automatic, the slider width is set to the visible
	 * height of the work window, and the maximum is set to the entire height of
	 * the work window. If the value becomes greater than the maximum minus the
	 * slider size, the value is reduced and the work window is scrolled up
	 * (i.e. moved down).
	 */
	if (horzAutomatic) {
		int maximum = workWidth;
		int sliderSize =
		        workWidth < clipWindow->width ? workWidth : clipWindow->width;
		...
		XtVaSetValues (horzBar, XmNmaximum, maximum, XmNsliderSize, sliderSize,
		        XmNvalue, value, XmNpageIncrement,
		        clipWindow->width - (CELL_HEIGHT - 1), NULL);
	}
	if (vertAutomatic) { /* Automatic? */
		int maximum = workHeight;
		int sliderSize = workHeight < clipWindow->height ? workHeight
		                                                 : clipWindow->height;
		...
		XtVaSetValues (vertBar, XmNmaximum, maximum, XmNsliderSize, sliderSize,
		        XmNvalue, value, XmNpageIncrement,
		        clipWindow->height - (CELL_HEIGHT - 1), NULL);
	}
}
...
static void xt_addCallback (
        XtCallbackList *callbacks, XtCallbackProc proc, XtPointer closure) {
	int i;
	...
	Melder_assert (
	        i < MAXNUM_CALLBACKS);   // will always fail if we arrive here
}

void XtAddCallback (
        GuiObject me, int kind, XtCallbackProc proc, XtPointer closure) {
	switch (kind) {
	case XmNactivateCallback:
		my activateCallback = proc;
		my activateClosure = closure;
		break;
	case XmNdestroyCallback:
		my destroyCallback = proc;
	...
	case XmNvalueChangedCallback:
		if (my widgetClass == xmScrollBarWidgetClass)
			xt_addCallback (
			        &my motiff.scrollBar.valueChangedCallbacks, proc, closure);
		else if (my widgetClass == xmToggleButtonWidgetClass)
			xt_addCallback (&my motiff.toggleButton.valueChangedCallbacks, proc,
			        closure);
		else
			Melder_assert (False);
		break;
...
void XtDestroyWidget (GuiObject me) {
	...
	while (subview) {
		...
		// if (subview -> widgetClass != xmShellWidgetClass)   /* FIX instead of
		// Xm's being_destroyed mark. */   // removed 20090105
...
static void mapWidget (GuiObject me) {
	...
	if (my inMenu) {
		...
		case xmPushButtonWidgetClass: {
			InsertMenu (my nat.entry.handle, position,
			        MF_STRING | MF_BYPOSITION |
			                (my insensitive ? MF_GRAYED : MF_ENABLED),
			        my nat.entry.id,
			        Melder_peek32toW (
			                _GuiWin_expandAmpersands (my name.get ())));
		} break;
		case xmToggleButtonWidgetClass: {
			InsertMenu (my nat.entry.handle, position,
			        MF_STRING | MF_UNCHECKED | MF_BYPOSITION |
			                (my insensitive ? MF_GRAYED : MF_ENABLED),
			        my nat.entry.id,
			        Melder_peek32toW (
			                _GuiWin_expandAmpersands (my name.get ())));
		...
		case xmSeparatorWidgetClass: {
			InsertMenu (my nat.entry.handle, position,
			        MF_SEPARATOR | MF_BYPOSITION, my nat.entry.id,
			        Melder_peek32toW (
			                _GuiWin_expandAmpersands (my name.get ())));
		} break;
		}
		...
		case xmMenuBarWidgetClass: {
			DrawMenuBar (
			        my shell->window);   // every window has its own menu bar
		} break;
		case xmPushButtonWidgetClass:
		...
		case xmScrollBarWidgetClass: {
			if (!my window) {
				my window = CreateWindow (L"scrollbar",
				        Melder_peek32toW (my name.get ()),
				        WS_CHILD |
				                (my orientation == XmHORIZONTAL ? SBS_HORZ
				                                                : SBS_VERT) |
				                WS_CLIPSIBLINGS,
				        my x, my y, my width, my height, my parent->window,
				        (HMENU) 1, theGui.instance, NULL);
...
void XtManageChild (GuiObject me) {
	...
	if (my parent && MEMBER (my parent, Shell) &&
	        MEMBER2 (me, Form, BulletinBoard)) {
		my parent->managed = 1;
		mapWidget (my parent);
	} else if (my inMenu) {
		mapWidget (me);
		...
		for (widget = me; widget != NULL; widget = widget->parent) {
			if (!widget->managed &&   // if a parent is invisible, so are its
			                          // children
			        !MEMBER (widget, PulldownMenu))   // the exception: "shown"
			                                          // even if not popped up
			{
				visible = False;
				break;
			}
...
void XtManageChildren (GuiObjectList children, Cardinal num_children) {
	for (Cardinal i = 0; i < num_children; i++) XtManageChild (children[i]);
}
...
void XtSetSensitive (GuiObject me, Boolean value) {
	...
	case xmPulldownMenuWidgetClass: {
		...
		if (my popUpButton) XtSetSensitive (my popUpButton, value);
	} break;
	default:
		break;
	}
...
void XtUnmanageChildren (GuiObjectList children, Cardinal num_children) {
	for (Cardinal i = 0; i < num_children; i++) XtUnmanageChild (children[i]);
}
...
void *GuiWin_initialize1 (conststring32 name) {
	Melder_sprint (theApplicationClassName, 100, U"PraatShell",
...
void GuiWin_initialize2 (unsigned int argc, char **argv) {
	...
	windowClass.hbrBackground =
	        /*(HBRUSH) (COLOR_WINDOW + 1)*/ GetStockBrush (LTGRAY_BRUSH);
	...
	windowClass.lpszClassName =
	        Melder_32toW (theDrawingAreaClassName).transfer ();
	RegisterClassEx (&windowClass);
	windowClass.lpszClassName =
...
GuiObject XtVaCreateManagedWidget (
        const char *name, int widgetClass, GuiObject parent, ...) {
	GuiObject me;
	va_list arg;
	va_start (arg, parent);
	...
	while ((resource = va_arg (arg, int)) != 0) switch (resource) {
		...
		case XmNy:
			*va_arg (arg, int *) = my y;
			break;
		case XmNwidth:
			*va_arg (arg, int *) = my width;
			break;
		case XmNheight:
			*va_arg (arg, int *) = my height;
			break;
		case XmNuserData:
			*va_arg (arg, void **) = my userData;
			break;
		case XmNtitle:
		...
		case XmNbottomOffset:
			...
			*va_arg (arg, int *) = my bottomOffset;
			break;
		case XmNminimum:
			*va_arg (arg, int *) = my minimum;
			break;
		case XmNmaximum:
			*va_arg (arg, int *) = my maximum;
			break;
		case XmNvalue:
			*va_arg (arg, int *) = my value;
			break;
		case XmNincrement:
			Melder_assert (my widgetClass == xmScrollBarWidgetClass);
		...
		case XmNcolumns:
			*va_arg (arg, int *) = 40;
			break;
		case XmNhorizontalScrollBar:
			*va_arg (arg, GuiObject *) = my motiff.scrolledWindow.horizontalBar;
			break;
		case XmNverticalScrollBar:
			*va_arg (arg, GuiObject *) = my motiff.scrolledWindow.verticalBar;
			break;
		default: {
			if (resource < 0 ||
			        resource >= sizeof motif_resourceNames / sizeof (char *))
...
Window XtWindow (GuiObject me) { return (Window) my window; }

/***** MOTIF *****/

void XmAddWMProtocolCallback (
        GuiObject me, Atom protocol, XtCallbackProc callback, char *closure) {
	if (protocol == 'delw') {
...
GuiObject XmCreateCascadeButtonGadget (
        GuiObject parent, const char *name, ArgList dum1, int dum2) {
	(void) dum1;
	(void) dum2;
...
GuiObject XmCreateFormDialog (
        GuiObject parent, const char *name, ArgList dum1, int dum2) {
	GuiObject shell = XmCreateDialogShell (parent, name, dum1, dum2);
	return XmCreateForm (shell, name, dum1, dum2);
}

GuiObject XmCreateMenuBar (
        GuiObject parent, const char *name, ArgList dum1, int dum2) {
	(void) dum1;
	(void) dum2;
...
GuiObject XmCreatePulldownMenu (
        GuiObject parent, const char *name, ArgList dum1, int dum2) {
	(void) dum1;
	(void) dum2;
...
GuiObject XmCreateScrolledWindow (
        GuiObject parent, const char *name, ArgList dum1, int dum2) {
	(void) dum1;
	(void) dum2;
...
GuiObject XmCreateSeparatorGadget (
        GuiObject parent, const char *name, ArgList dum1, int dum2) {
	(void) dum1;
	(void) dum2;
	return createWidget (xmSeparatorWidgetClass, parent, name);
...
GuiObject XmCreateToggleButtonGadget (
        GuiObject parent, const char *name, ArgList dum1, int dum2) {
	(void) dum1;
	(void) dum2;
...
void XmScrollBarSetValues (GuiObject me, int value, int sliderSize,
	...
	if (notify)
		_Gui_callCallbacks (
		        me, &my motiff.scrollBar.valueChangedCallbacks, NULL);
}

Boolean XmToggleButtonGadgetGetState (GuiObject me) {
	Melder_assert (MEMBER (me, ToggleButton));
	Melder_assert (my inMenu);
	return GetMenuState (my nat.entry.handle, my nat.entry.id, MF_BYCOMMAND) &
	                       MF_CHECKED
	               ? True
	               : False;
}
...
void XmToggleButtonGadgetSetState (
        GuiObject me, Boolean value, Boolean notify) {
	Melder_assert (MEMBER (me, ToggleButton));
	Melder_assert (my inMenu);
	NativeMenuItem_check (me, value);
	if (notify)
		_Gui_callCallbacks (
		        me, &my motiff.toggleButton.valueChangedCallbacks, NULL);
}
...
static void _motif_inspectTextWidgets (GuiObject me, GuiObject text,
	...
	for (GuiObject sub = my firstChild; sub != NULL; sub = sub->nextSibling) {
		...
		if (MEMBER (sub, Text)) {
			...
			if (sub == text) *p_textWidgetLocation = *p_numberOfTextWidgets;
		} else {
			_motif_inspectTextWidgets (
			        sub, text, p_numberOfTextWidgets, p_textWidgetLocation);
		}
	}
}
static GuiObject _motif_getLocatedTextWidget (
        GuiObject me, integer *p_itextWidget, integer textWidgetLocation) {
	for (GuiObject sub = my firstChild; sub != NULL; sub = sub->nextSibling) {
		if (MEMBER (sub, Shell)) continue;
		if (MEMBER (sub, Text)) {
			(*p_itextWidget)++;
			if (*p_itextWidget == textWidgetLocation) return sub;
		} else {
			GuiObject locatedTextWidget = _motif_getLocatedTextWidget (
			        sub, p_itextWidget, textWidgetLocation);
			if (locatedTextWidget) return locatedTextWidget;
		}
	}
	return NULL;
}
static GuiObject _motif_getNextTextWidget (
        GuiObject shell, GuiObject text, bool backward) {
	integer numberOfTextWidgets = 0, textWidgetLocation = 0;
	_motif_inspectTextWidgets (
	        shell, text, &numberOfTextWidgets, &textWidgetLocation);
	trace (U"Found ", numberOfTextWidgets, U" text widgets.");
	if (numberOfTextWidgets == 0)
		return nullptr;   // no tab navigation if there is no text widget
		                  // (shouldn't normally occur)
	Melder_assert (textWidgetLocation >= 1);
	Melder_assert (textWidgetLocation <= numberOfTextWidgets);
	if (numberOfTextWidgets == 1)
		return NULL;   // no tab navigation if there is only one text widget
	if (backward) {
		textWidgetLocation--;   // tab to previous text widget
		if (textWidgetLocation < 1)
			textWidgetLocation =
			        numberOfTextWidgets;   // if at beginning, then tab around
			                               // to last text widget
	} else {
		textWidgetLocation++;   // tab to next text widget
		if (textWidgetLocation > numberOfTextWidgets)
			textWidgetLocation =
			        1;   // if at end, then tab around to first text widget
	}
	integer itextWidget = 0;
	return _motif_getLocatedTextWidget (
	        shell, &itextWidget, textWidgetLocation);
}
...
static void on_scroll (GuiObject me, UINT part, int pos) {
	...
	case SB_THUMBPOSITION:
		my value = my minimum +
		           pos * ((my maximum - my minimum - my sliderSize) /
		                         (32767.0 - (32767.0 * my sliderSize) /
	...
	if (part == SB_THUMBTRACK || part == SB_THUMBPOSITION)
		_Gui_callCallbacks (me, &my motiff.scrollBar.dragCallbacks,
		        (XtPointer) (ULONG_PTR) part);
	else
		_Gui_callCallbacks (me, &my motiff.scrollBar.valueChangedCallbacks,
		        (XtPointer) (ULONG_PTR) part);
}

void XtNextEvent (XEvent *xevent) { GetMessage (xevent, NULL, 0, 0); }
...
modifiers & _motif_SHIFT_MASK ? " shift" : "", message -> message == WM_KEYDOWN ? "keydown" : "syskeydown", kar);
	 ...
	 * backslash) on some (e.g. German) keyboards; in such a case, the message
	 * is WM_KEYDOWN, and the reported modifier keys are Control (!) and Alt (on
		...
		message -> message == WM_SYSKEYDOWN && GetKeyState (VK_CONTROL) < 0)
	{
		...
		modifiers & _motif_COMMAND_MASK ? " control" : "",
		modifiers & _motif_OPTION_MASK ? " alt" : "",
		modifiers & _motif_SHIFT_MASK ? " shift" : "", message -> message ==
		WM_KEYDOWN ? "keydown" : "syskeydown", kar);*/
		if (me && my shell) {
			...
			if (kar < 48) {
				if (kar == VK_BACK) {   // shortcut or text
					if (acc & 1 << GuiMenu_BACKSPACE) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_BACKSPACE, modifiers);
						return;
					}
				} else if (kar == VK_TAB) {   // shortcut or text
					if (acc & 1 << GuiMenu_TAB) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_TAB, modifiers);
						return;
					}
				} else if (kar ==
				           VK_RETURN) {   // shortcut, default button, or text
					// Melder_information (U"RETURN ", acc, U" def ",
					// Melder_pointer (my shell -> defaultButton));
					if (acc & 1 << GuiMenu_ENTER) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_ENTER, modifiers);
						return;
					} else {
						if (my shell->defaultButton &&
					...
					if (acc & 1 << GuiMenu_PAGE_UP) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_PAGE_UP, modifiers);
						return;
					}
				} else if (kar == VK_NEXT) {   // shortcut or text
					if (acc & 1 << GuiMenu_PAGE_DOWN) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_PAGE_DOWN, modifiers);
						return;
					}
				} else if (kar == VK_HOME) {   // shortcut or text
					if (acc & 1 << GuiMenu_HOME) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_HOME, modifiers);
						return;
					}
				} else if (kar == VK_END) {   // shortcut or text
					if (acc & 1 << GuiMenu_END) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_END, modifiers);
						return;
					}
				} else if (kar == VK_LEFT) {   // shortcut or text
					if (acc & 1 << GuiMenu_LEFT_ARROW) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_LEFT_ARROW, modifiers);
						return;
					}
				} else if (kar == VK_RIGHT) {   // shortcut or text
					if (acc & 1 << GuiMenu_RIGHT_ARROW) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_RIGHT_ARROW, modifiers);
						return;
					}
				} else if (kar == VK_UP) {   // shortcut or text
					if (acc & 1 << GuiMenu_UP_ARROW) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_UP_ARROW, modifiers);
						return;
					}
				} else if (kar == VK_DOWN) {   // shortcut or text
					if (acc & 1 << GuiMenu_DOWN_ARROW) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_DOWN_ARROW, modifiers);
						return;
					}
				} else if (kar == VK_INSERT) {   // shortcut
					win_processKeyboardEquivalent (
					        my shell, GuiMenu_INSERT, modifiers);
					return;
				} else if (kar == VK_DELETE) {   // shortcut or text
					if (acc & 1 << GuiMenu_DELETE) {
						win_processKeyboardEquivalent (
						        my shell, GuiMenu_DELETE, modifiers);
						return;
					}
				} else if (kar == VK_HELP) {   // simulate Command-?
					win_processKeyboardEquivalent (
					        my shell, '?', modifiers | _motif_SHIFT_MASK);
					return;
				}
			 ...
			 */
				...
				if (MEMBER (me, Text) && (kar == 'X' || kar == 'C' || kar == 'V' || kar == 'Z')) {
					 ...
					 */
					if (kar == 186 && win_processKeyboardEquivalent (my shell,
					                          shift ? ':' : ';', modifiers) ||
					        kar == 187 &&
		...
		if (me && MEMBER (me, Text) && kar == 9) {
			trace (U"Tab navigation with shell ", Melder_pointer (my shell),
			        U" and from text widget ", Melder_pointer (me));
			GuiObject nextTextWidget = _motif_getNextTextWidget (
			        my shell, me, GetKeyState (VK_SHIFT) < 0);
			trace (U"Tab navigation to text widget ",
			        Melder_pointer (nextTextWidget));
			if (nextTextWidget != NULL) {
				_GuiText_setTheTextFocus (nextTextWidget);
				GuiText_setSelection (
				        (GuiText) nextTextWidget->userData, 0, 10000000);
				return;
			}
		}
		...
		GuiObject me =
		        (GuiObject) GetWindowLongPtr (message->hwnd, GWLP_USERDATA);
		// Melder_information (Melder_pointer (me), U" -- ", Melder_pointer (my
		// subMenuId));
		if (me && MEMBER (me, CascadeButton) && my subMenuId) {
			RECT rect;
			GetWindowRect (my window, &rect);
			TrackPopupMenu (my subMenuId->nat.menu.handle,
			        TPM_LEFTBUTTON | TPM_LEFTALIGN | TPM_TOPALIGN, rect.left,
			        rect.bottom - 3, 0, my parent->window, NULL);
			return;
		}
	}
...
static void on_close (HWND window) {
	...
	if (me) {
		if (my widgetClass == xmShellWidgetClass) {
			int deleteResponse = my
			        deleteResponse;   // save this, in case the callback should
			                          // kill the widget (XmDO_NOTHING)
			GuiObject parent = my parent;
			if (my motiff.shell.goAwayCallback)
				my motiff.shell.goAwayCallback (
				        me, my motiff.shell.goAwayClosure, NULL);
			if (deleteResponse == XmDESTROY) {
				XtDestroyWidget (me);
			} else if (deleteResponse == XmUNMAP) {
				/*
				    Unmapping is not the same as unmanaging.
				    One of the oldest bugs in Praat was that in the Windows
				   edition the Picture window could not be closed a second time,
				    and that after it had been closed and opened the scroll bars
				   did not move when the user resized the window. The solution
				   was to use only some of the code from XtUnmanageChild here,
			...
			FORWARD_WM_CLOSE (window, DefWindowProc);
	} else
		FORWARD_WM_CLOSE (window, DefWindowProc);
}
static GuiObject findItem (GuiObject menu, int id) {
	...
	for (child = menu->firstChild; child != NULL; child = child->nextSibling) {
		if (child->widgetClass == xmPulldownMenuWidgetClass) {
			...
			Melder_assert (MEMBER4 (
			        child, PushButton, ToggleButton, CascadeButton, Separator));
...
static void on_command (
	...
	if (parent) {
		if (controlWindow) {
			GuiObject control =
			        (GuiObject) GetWindowLongPtr (controlWindow, GWLP_USERDATA);
			if (control) {
				 ...
				 * We have to put the focus back (see GuiText.c, parenthesized
				 * discussion 1.4); this has to be done before calling the
				 * callbacks, because these may destroy or hide the text widget.
				 */
				case xmPushButtonWidgetClass:
					/*
				...
				case xmTextWidgetClass:
					if (codeNotify == EN_CHANGE)
						...
						_GuiText_handleFocusLoss (
						        control);   // for button clicks (see above)
					break;
				default:
					break;
				}
				...
				if (item) {
					if (item->widgetClass == xmPushButtonWidgetClass) {
						if (item->activateCallback)
							item->activateCallback (
							        item, item->activateClosure, 0);
					} else if (item->widgetClass == xmToggleButtonWidgetClass) {
						XmToggleButtonGadgetSetState (item,
						        1 - XmToggleButtonGadgetGetState (item), False);
						_Gui_callCallbacks (item,
						        &item->motiff.toggleButton
	...
	if (me) {
		if (MEMBER (me, DrawingArea) && (flags & MK_LBUTTON)) {
			_GuiWinDrawingArea_handleMouse (
			        me, structGuiDrawingArea_MouseEvent::Phase::DRAG, x, y);
		} else
			FORWARD_WM_MOUSEMOVE (window, x, y, flags, DefWindowProc);
	} else
		FORWARD_WM_MOUSEMOVE (window, x, y, flags, DefWindowProc);
}
...
static void on_paint (HWND window) {
	...
	if (me) {
		if (my widgetClass == xmDrawingAreaWidgetClass) {
			...
			FORWARD_WM_PAINT (window, DefWindowProc);
	} else
		FORWARD_WM_PAINT (window, DefWindowProc);
}
static void on_hscroll (HWND window, HWND controlWindow, UINT code, int pos) {
	...
	if (parent) {
		...
		FORWARD_WM_HSCROLL (window, controlWindow, code, pos, DefWindowProc);
}
static void on_vscroll (HWND window, HWND controlWindow, UINT code, int pos) {
	...
	if (parent) {
		...
		FORWARD_WM_VSCROLL (window, controlWindow, code, pos, DefWindowProc);
}
...
/* --------------------------------------------
	...
	O parãmetro de aumento/diminuiçao é dada pela equação:
		enlargement = exp (-0.02 * (zDelta > 0.0 ? +1 : -1) * sqrt (fabs (zDelta)));
	
	Mudança: 
	Rodando a roda para baixo desloca a janela para a esquerda (avança o tempo da janela).
	Rodando a roda para cima desloca a janela para a direita (retrocede o tempo da janela).
	comportamento inverso da função original, mas mais intuitivo para o usuário.
	Zoom:
	Rodando a roda para baixo + CRTL aumenta o zoom (diminui a escala do tempo).
	Rodando a roda para cima + CRTL diminui o zoom (aumenta a escala do tempo).
	Comportamento inverso ao da função original.
*/
static void on_mouseWheel (HWND window, int xPos, int yPos, int zDelta, int fwKeys) {
	GuiObject me = (GuiObject) GetWindowLongPtr (window, GWLP_USERDATA);
	if (me) {
		if (my widgetClass == xmDrawingAreaWidgetClass) {
			zDelta = - zDelta;   // inverte a direção, para ficar mais intuitivo
...
static void on_size (HWND window, UINT state, int cx, int cy) {
	...
	if (me && MEMBER (me, Shell) &&
	        (state == SIZE_RESTORED || state == SIZE_MAXIMIZED)) {
		...
		if (!my managed) {
		} else if (my nat.shell.duringMoveWindow)   // Yeah, a BUG fix. Only
		                                            // react to user actions.
			my nat.shell.duringMoveWindow = False;
		else if (newWidth != oldWidth || newHeight != oldHeight) {
			shellResizeWidget (
			        me, 0, 0, newWidth - oldWidth, newHeight - oldHeight);
		}
	} else
		FORWARD_WM_SIZE (window, state, cx, cy, DefWindowProc);
}
static void on_key (HWND window, UINT key, BOOL down, int repeat, UINT flags) {
	...
	if (me && key >= VK_LEFT && key <= VK_DOWN) {
		...
		if (MEMBER (me, Shell) ||
		        my widgetClass ==
		                xmDrawingAreaWidgetClass) {   // any change in this
			                                          // condition should be
			                                          // mirrored in `on_char`
...
static void on_move (HWND window, int x, int y) {
	GuiObject me = (GuiObject) GetWindowLongPtr (window, GWLP_USERDATA);
	/*if (me && MEMBER (me, Shell)) {
	    my x = x - ( my motiff.shell.isDialog ? GetSystemMetrics
	(SM_CXFIXEDFRAME) : GetSystemMetrics (SM_CXSIZEFRAME) ); my y = y -
	GetSystemMetrics (SM_CYCAPTION) - ( my motiff.shell.isDialog ?
	GetSystemMetrics (SM_CYFIXEDFRAME) : GetSystemMetrics (SM_CYSIZEFRAME) +
	GetSystemMetrics (SM_CYMENU) );
	}*/
	FORWARD_WM_MOVE (window, x, y, DefWindowProc);
}
static HBRUSH on_ctlColorStatic (
        HWND window, HDC hdc, HWND controlWindow, int type) {
	GuiObject parent = (GuiObject) GetWindowLongPtr (window, GWLP_USERDATA);
	(void) type;
	if (parent) {
		GuiObject control =
	...
	return FORWARD_WM_CTLCOLORSTATIC (
	        window, hdc, controlWindow, DefWindowProc);
}
static HBRUSH on_ctlColorBtn (
        HWND window, HDC hdc, HWND controlWindow, int type) {
	GuiObject parent = (GuiObject) GetWindowLongPtr (window, GWLP_USERDATA);
	(void) type;
	if (parent) {
...
static void on_activate (
        HWND window, UINT state, HWND hActive, BOOL minimized) {
	GuiObject me = (GuiObject) GetWindowLongPtr (window, GWLP_USERDATA);
