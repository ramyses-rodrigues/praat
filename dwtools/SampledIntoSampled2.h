#ifndef _SampledIntoSampled2_h_
#define _SampledIntoSampled2_h_
/* SampledIntoSampled2.h
 *
...
/*
	...
	SampledFrameIntoSampledFrame2.h, SoundFrameIntoSampledFrame2.h, 
	Sound_and_LPC2.h, Sound_and_LPC2.cpp
...
inline void SampledIntoSampled2_assertEqualSampling (constSampled me,  constSampled thee) {
	Melder_assert (me && thee);
	Melder_assert (my x1 == thy x1 && my nx == thy nx && my dx == thy dx);
}

inline void SampledIntoSampled2_requireEqualDomains (constSampled me,  constSampled thee) {
	Melder_assert (me && thee);
	Melder_require (my xmin == thy xmin && my xmax == thy xmax,
		U"The domains of ", me, U" and ", thee, U" should be equal.");
}

inline void SampledIntoSampled2_assertEqualDomains (constSampled me,  constSampled thee) {
	Melder_assert (me && thee);
	Melder_assert (my xmin == thy xmin && my xmax == thy xmax);
}

inline void SampledIntoSampled2_requireEqualDomainsAndSampling (constSampled me,  constSampled thee) {
	Melder_assert (me && thee);
	SampledIntoSampled2_requireEqualDomains (me, thee);
	Sampled_requireEqualSampling (me, thee);
}

inline void SampledIntoSampled2_assertEqualDomainsAndSampling (constSampled me,  constSampled thee) {
	Melder_assert (me && thee);
	SampledIntoSampled2_assertEqualDomains (me, thee);
	SampledIntoSampled2_assertEqualSampling (me, thee);
}

/*********** timing only *********/

void SampledIntoSampled2_timeMultiThreading (double soundDuration);

#endif /* _SampledIntoSampled2_h_ */
